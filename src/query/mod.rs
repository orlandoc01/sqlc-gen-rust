...
/// Every slot layout downstream (`arg_slots`, `-- :if $N` markers, condition and flag slots)
/// indexes by `number - 1` and starts flag slots at `params.len()`, so a gap or duplicate would
/// silently gate or bind the wrong argument.
fn validate_contiguous_numbers(query_name: &str, numbers: &[usize]) -> Result<(), QueryError> {
    let mut sorted = numbers.to_vec();
    sorted.sort_unstable();
    if sorted.iter().copied().eq(1..=numbers.len()) {
        return Ok(());
    }
    Err(QueryError::dynamic_filter(
        query_name.to_string(),
        format!(
            "sqlc reported parameter numbers {numbers:?}, which are not 1..={}",
            numbers.len()
        ),
    ))
}
...
impl Query {
    pub(crate) fn from_query(
        ...
        let param_numbers = query
            ...
            .collect::<Vec<_>>();
        validate_contiguous_numbers(&query_name, &param_numbers)?;
    ...
    pub(crate) fn arg_order(&self) -> &[usize] {
        &self.param_numbers
    }
