...
fn dedups_collapsed_states_and_keeps_switches_exclusive() {
    ...
    let variants = expand(&query, Placeholders::Numbered, 1024, None).unwrap();
    ...
    let again = expand(&query, Placeholders::Numbered, 1024, None).unwrap();
    assert!(
        variants
            .iter()
            .zip(&again)
...
fn renders_engine_placeholders_and_single_element_slices() {
    ...
    let variants = expand(&sqlite, Placeholders::NumberedSqlite, 1024, None).unwrap();
    ...
    let variants = expand(&mysql, Placeholders::Question, 1024, None).unwrap();
    assert_eq!(
        variants[3].sql,
        "SELECT id FROM users\nWHERE\n  email = ?\n  AND id IN (?)"
    );
    ...
    let variants = expand(&postgres, Placeholders::Numbered, 1024, None).unwrap();
    assert_eq!(
        variants[1].sql,
        "SELECT id FROM users\nWHERE\n  email = $1\n  AND phone = $1"
    );
...
fn enforces_limit_and_skip_list() {
    let query = postgres("SearchUsers", SEARCH_USERS, SEARCH_USERS_PARAMS);
    let error = expand(&query, Placeholders::Numbered, 10, None)
        .unwrap_err()
        .to_string();
    assert_eq!(
        error,
    ...
    let deduped = expand(&query, Placeholders::Numbered, 1024, None)
        .unwrap()
        .len();
    assert_eq!(
        expand(&query, Placeholders::Numbered, deduped, None)
            .unwrap()
            .len(),
        deduped
    );
    assert_eq!(
        expand(&query, Placeholders::Numbered, 0, None)
            .unwrap_err()
            .to_string(),
...
fn enumerates_queries_whose_only_controls_are_flags_or_switches() {
    ...
    let sql = |query: &Query| {
        expand(query, Placeholders::Numbered, 1024, None)
            .unwrap()
            .into_iter()
...
fn caps_raw_control_states_before_rendering() {
    ...
    let error = expand(&query, Placeholders::Numbered, usize::MAX, None)
        .unwrap_err()
        .to_string();
    assert_eq!(
        error,
    ...
    let error = expand(&query, Placeholders::Numbered, 3, None)
        .unwrap_err()
        .to_string();
...
fn collapsing_states_record_alternates_by_type_class_only_when_asked() {
    ...
    let started = std::time::Instant::now();
    let plain = expand(&query, Placeholders::Numbered, 1024, None).unwrap();
    ...
    let classed = expand(&query, Placeholders::Numbered, 1024, Some(&same_class)).unwrap();
    ...
    mixed[15] = 1;
    let mixed = expand(&query, Placeholders::Numbered, 1024, Some(&mixed)).unwrap();
