...
fn allocates_controls_after_every_sql_argument_even_with_duplicate_names() {
    ...
    assert_eq!(resolved.arg_index(&params, "hide_archived"), Some(2));
...
fn classifies_directives_into_conditions_flags_and_switch_slots() {
    ...
    assert_eq!(references.arg_index(&params, "email"), Some(0));
    assert_eq!(references.arg_index(&params, "has_orders"), Some(2));
    assert_eq!(references.arg_index(&params, "id_desc"), Some(4));
    assert_eq!(references.arg_index(&params, "active"), Some(5));
    assert_eq!(references.arg_index(&params, "sort"), None);
}
