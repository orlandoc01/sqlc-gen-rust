...
impl<'a> Resolver<'a> {
    fn scan(&mut self, node: &impl Visit) -> Result<(), String> {
        self.scan_with(node, Aliases::None)
    }

    fn scan_with(&mut self, node: &impl Visit, aliases: Aliases<'_>) -> Result<(), String> {
        let mut scan = Scan {
            resolver: self,
            aliases,
            nested: 0,
                ...
                for term in terms {
                    match term {
                        Expr::Identifier(ident) => {
                            self.bare(ident, Aliases::AfterColumns(aliases))?
                        }
                        term => self.scan(term)?,
                    }
                }
    ...
    fn having(&mut self, having: Option<&Expr>, aliases: &[String]) -> Result<(), String> {
        ...
        match self.dialect {
            ...
            Dialect::MySql | Dialect::Sqlite => {
                self.scan_with(having, Aliases::AfterColumns(aliases))
            }
        }
    }
    ...
    /// column binds first.
    fn bare(&mut self, ident: &Ident, aliases: Aliases<'_>) -> Result<(), String> {
        let name = self.dialect.fold(ident);
        let at = ident.span.start;
        let alias = matches!(aliases, Aliases::AfterColumns(aliases) if aliases.contains(&name));
...
/// Projection aliases a clause may resolve after input columns.
#[derive(Clone, Copy)]
enum Aliases<'r> {
    None,
    AfterColumns(&'r [String]),
}
