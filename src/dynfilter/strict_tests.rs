...
mod switches {
    ...
    /// Prose comments and blank lines outside dynamic clauses reach the query text, so query
    /// tags stay visible to the server; comments inside a dynamic clause or on a gated line
    /// are dropped, since a marker or connector must end those lines.
    #[test]
    ...
    /// A comment on a gated line or on an item of a dynamic clause is dropped: the marker
    /// (or an ORDER BY comma) must end the line, and a comment following a `?` would hide a
    /// MySQL bind from the runtime.
    #[test]
    fn comments_on_gated_lines_and_dynamic_items_are_dropped() {
        let info = parse_uncatalogued(
            "SELECT id FROM t\nWHERE\n  active = 1 -- always\n  AND name = ? -- :if @name\nORDER BY\n  id ASC, -- newest last\n  name DESC -- :flag @by_name",
            &params(&["name"]),
            Dialect::MySql,
        )
        .unwrap()
        ...
        // Trailing whitespace after a comment sits outside the trimmed item range, but the
        // comment still goes, so the ORDER BY comma lands on SQL.
        let info = parse_uncatalogued(
            "SELECT id FROM t\nORDER BY\n  a -- note   \n  , b -- :flag @f",
            &params(&[]),
            Dialect::PostgreSql,
        ...
        // A blank line inside a gated structure belongs to it and is dropped with the rest.
        let info = parse_uncatalogued(
            "SELECT id FROM t\nWHERE\n  id IN (\n    1,\n\n    2\n  ) -- :flag @x\nORDER BY id",
            &params(&[]),
            Dialect::PostgreSql,
        ...
        .unwrap();
        assert_eq!(
            compile(&info, Placeholders::Numbered, &[])
                .build(&[Arg::Flag(false)])
                .0,
            "SELECT id FROM t\nORDER BY id"
        );
        let info = parse_uncatalogued(
            "SELECT id FROM t\nWHERE EXISTS ( -- :flag @has_orders\n  SELECT 1 FROM orders -- any order\n  WHERE orders.t_id = t.id\n)",
            &params(&[]),
            Dialect::PostgreSql,
        )
        .unwrap()
...
mod multiline_clauses {
    use crate::dynfilter::Dialect;
    use crate::dynfilter::test_support::{params, parse_uncatalogued};

    /// The runtime copies an open literal's continuation lines through verbatim, so a value
    /// spanning lines inside a clause with dynamic items would be lexed as SQL or keep a
    /// connector the runtime strips.
    #[test]
    fn rejects_multiline_values_inside_clauses_with_dynamic_items() {
        for (sql, dialect) in [
            (
                "SELECT id FROM t\nWHERE\n  note = 'line1\n? line2'\n  AND name = ? -- :if @name",
                Dialect::MySql,
            ...
            let error = parse_uncatalogued(sql, &params(&["name"]), dialect).unwrap_err();
            assert!(
                error.contains("has dynamic items and a multi-line literal or comment"),
                "{sql}: {error}"
            );
        }
        let error = parse_uncatalogued(
        ...
        .unwrap_err();
        assert!(
            error.starts_with("an OR group on line 4 has dynamic items"),
            "{error}"
        );
        let error = parse_uncatalogued(
            "SELECT id FROM t\nWHERE\n  \"odd\n name\" = 1\n  AND name = $1 -- :if @name",
        ...
        .unwrap_err();
        assert!(
            error.starts_with("a WHERE clause on line 2 has dynamic items")
                && error.contains("starting on line 3"),
            "{error}"
        );
        // Outside such a clause the value is data and passes through untouched.
        parse_uncatalogued(
            "SELECT 'a\nb' AS label FROM t\nWHERE\n  name = $1 -- :if @name",
            &params(&["name"]),
            Dialect::PostgreSql,
...
mod clause_extents {
    ...
    /// sqlparser tags every word with its keyword id whether or not the dialect reserves it,
    /// so the clause extent comes from the parsed items, not from a keyword scan.
    #[test]
    fn columns_named_like_clause_keywords_stay_inside_the_clause() {
        let info = parse_uncatalogued(
            "SELECT id FROM users\nWHERE\n  email = ? -- :if @email\n  AND returning = 1\n  AND fetch > 0\nORDER BY\n  window DESC, -- :flag @by_window\n  id ASC\nLIMIT 1",
            &params(&["email"]),
            Dialect::MySql,
        )
        .unwrap()
        .unwrap();
        let compiled = compile(&info, Placeholders::Question, &[1]);
        assert_eq!(
            compiled.build(&[Arg::Active, Arg::Flag(true)]),
            (
                "SELECT id FROM users\nWHERE\n  email = ?\n  AND returning = 1\n  AND fetch > 0\nORDER BY\n  window DESC,\n  id ASC\nLIMIT 1".to_string(),
                vec![Bind::Arg(0)]
            )
        ...
        .unwrap();
        assert_eq!(
            compile(&info, Placeholders::Question, &[1])
                .build(&[Arg::Inactive])
                .0,
            "SELECT id FROM users\nWHERE\n  returning = 1\nLIMIT 1"
        );
    ...
    /// The opener is the nearest WHERE or ORDER BY before the first item, so a subquery's own
    /// clauses inside the first item, or a window `ORDER BY` inside a term, never claim it.
    #[test]
    fn nested_clauses_inside_the_first_item_do_not_move_the_opener() {
        let info = parse_uncatalogued(
            "SELECT id FROM users\nWHERE\n  id IN (SELECT user_id FROM orders WHERE total > 0 ORDER BY total LIMIT 5)\n  AND email = $1 -- :if @email\nORDER BY\n  rank() OVER (PARTITION BY id ORDER BY id) ASC,\n  id DESC -- :flag @newest",
            &params(&["email"]),
            Dialect::PostgreSql,
        )
        .unwrap()
        .unwrap();
        assert_eq!(
            compile(&info, Placeholders::Numbered, &[1])
                .build(&[Arg::Inactive, Arg::Flag(false)])
                .0,
            "SELECT id FROM users\nWHERE\n  id IN (SELECT user_id FROM orders WHERE total > 0 ORDER BY total LIMIT 5)\nORDER BY\n  rank() OVER (PARTITION BY id ORDER BY id) ASC"
        );
    }
    ...
    #[test]
    fn real_clause_keywords_after_the_items_still_end_the_clause() {
        let info = parse_uncatalogued(
        ...
        .unwrap();
        assert_eq!(
            compile(&info, Placeholders::Numbered, &[1, 2])
                .build(&[Arg::Inactive, Arg::Active])
                .0,
