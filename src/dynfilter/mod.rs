...
impl DynFilters {
    /// Skip lists need the flag, since enumeration only runs with it on; every cache-skipped
    /// name must be a dynamic query (`variants_skip` is checked by the enumerator).
    pub(crate) fn validate(&self, queries: &[crate::query::Query]) -> Result<(), crate::Error> {
        if !self.prepared {
            for (option, list) in [
        ...
        variants::validate_skip_names(queries, &self.prepared_skip, "dynfilters.prepared_skip")?;
        Ok(())
    }
}
...
pub(crate) struct References {
    ...
    pub(crate) flag_params: Vec<FlagParam>,
    pub(crate) switches: Vec<Switch>,
    pub(crate) arg_index_by_name: HashMap<String, usize>,
}
