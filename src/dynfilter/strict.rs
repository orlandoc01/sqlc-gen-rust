...
use super::{
    ...
    source::{MultilineTokens, Range, Source, first_non_whitespace},
    structures::{AstStructures, Candidate, Clause, Unsupported, covered, structures_from_ast},
};
...
pub(crate) fn parse(
    ...
    for candidate in &mut candidates {
        candidate.conditions = candidate
            ...
            .map(|name| {
                references
                    .arg_index(params, name)
                    .expect("references resolved every annotation name")
            })
            .collect();
    }
    reject_multiline_tokens_in_dynamic_clauses(&source, &clauses, &candidates, &multiline)?;
...
/// Every line comment, directive or prose, cut at its line end.
...
/// The runtime re-lexes a dynamic clause line by line and copies the lines of an open literal
/// or comment through verbatim, so a value spanning lines inside such a clause would have its
/// continuation lexed as SQL (claiming a `?` inside it as a bind) or keep a connector the
/// runtime should have stripped. Gated block comments are blanked and never reach the runtime.
fn reject_multiline_tokens_in_dynamic_clauses(
    source: &Source<'_>,
    clauses: &[Clause],
    candidates: &[Candidate],
    multiline: &MultilineTokens,
) -> Result<(), String> {
    let gated = |range: Range| {
        candidates
            .iter()
            .any(|candidate| !candidate.conditions.is_empty() && candidate.range.contains(range))
    };
    let offending = clauses
        .iter()
        .filter(|clause| {
            clause
                .items
                .iter()
                .any(|item| !candidates[*item].conditions.is_empty())
        })
        .find_map(|clause| {
            multiline
                .literals
                .iter()
                .chain(multiline.comments.iter().filter(|range| !gated(**range)))
                .find(|range| clause.range.contains(**range))
                .map(|range| (clause, range))
        });
    match offending {
        Some((clause, range)) => Err(format!(
            "{} on line {} has dynamic items and a multi-line literal or comment starting on line {}; split the value or move it out of the clause",
            clause.kind.describe(),
            source.line_of(clause.keyword.start) + 1,
            source.line_of(range.start) + 1
        )),
        None => Ok(()),
    }
