//! Control slot allocation: every `:flag` and switch choice claims one argument slot after the
//! SQL parameters, in claim order, and `references` reports the plan's control layout.
