...
pub(super) struct Renderer<'a> {
    ...
    /// Every line comment, in source order. Dropped from gated lines and from dynamic
    /// clauses, where a gate marker or connector must end the line; kept elsewhere so query
    /// tags survive.
    pub(super) comments: &'a [Range],
    /// The annotation comments, in source order: always dropped.
    pub(super) directives: &'a [Range],
...
impl Renderer<'_> {
    ...
    /// `dynamic` marks text inside a dynamic clause's items.
    pub(super) fn fragment(&self, range: Range, dynamic: bool) -> Vec<Line> {
        let clauses = self
            .clauses
            .iter()
            .enumerate()
        ...
        for index in &clauses {
                ...
                .is_some_and(|character| character != '\n')
            {
                ...
                let mut rest = self.raw(Range { start: cursor, end }, dynamic);
    ...
    pub(super) fn clause(&self, index: usize) -> Vec<Line> {
        ...
        let items = clause
            ...
            .flat_map(|(position, candidate)| {
                ...
                let mut lines = self.fragment(candidate.range, true);
    ...
    pub(super) fn raw(&self, range: Range, dynamic: bool) -> Vec<Line> {
        let mut cursor = range.start;
        ...
        while cursor < range.end {
            let end = self.text[cursor..range.end]
                ...
                .map_or(range.end, |offset| cursor + offset);
            let whole_line =
                end < range.end && (cursor == 0 || self.text[..cursor].ends_with('\n'));
            let position = first_non_whitespace(self.text, cursor, end);
            let gates = gates_at(self.candidates, position.unwrap_or(cursor)).collect::<Vec<_>>();
            let strip_all = dynamic || !gates.is_empty();
            let text = self.clean(Range { start: cursor, end }, strip_all);
            ...
            } else if !text.trim().is_empty() || (position.is_none() && whole_line && !strip_all) {
                // A line left empty by a dropped comment goes with it; a blank source line
                // stays unless a gated structure or dynamic clause owns it.
                lines.push(Line {
                    role: Role::Plain,
                    text: self.line(text.trim_end(), &gates),
                });
            }
            cursor = end + 1;
        }
    ...
    pub(super) fn clean(&self, range: Range, all: bool) -> String {
        let mut cursor = range.start;
        let mut output = String::new();
        let comments = if all { self.comments } else { self.directives };
        // A comment runs to its line end, past the trailing whitespace an item range trims.
        for comment in comments
            .iter()
            .filter(|comment| range.contains_position(comment.start))
        {
            output.push_str(&self.text[cursor..comment.start]);
            cursor = comment.end.min(range.end);
        }
