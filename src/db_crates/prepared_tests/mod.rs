...
use crate::{
    db_crates::{
        DbCrate, GenerateOptions, Postgres, Sqlx,
        test_support::{column, dialect, parsed, query},
    },
    dynfilter::{
        DynFilters,
        prepared::Prepared,
...
impl Fixture {
    ...
    pub(super) fn generate(&self, mode: Mode) -> Result<String, QueryError> {
        let (rows, queries) = parsed(self.backend, &self.type_map, None, &self.queries);
        let prepared = match mode {
            Mode::Off => None,
            Mode::On => Some(Prepared::enumerate(
                &queries,
                self.backend,
                &self.dynfilters,
            )?),
...
fn eligible_queries_emit_variants_and_warmups() {
    for backend in DbCrate::ALL {
        ...
        if caches(backend) {
            ...
            assert!(tokens.contains("(sql . as_str () , & values)"), "{backend}");
        }
    }
}
        ...
        match backend {
            ...
            DbCrate::Postgres(Postgres::Deadpool) => {
                for cached in [eligible, by_ids] {
                    assert_eq!(
                        cached
                            .matches("prepare_cached (sql . as_str ()) . await ?")
                            .count(),
                        2,
                        "{cached}"
                    );
                ...
                assert_eq!(
                    ineligible.matches("(sql . as_str () , & values)").count(),
                    2
                );
                assert_eq!(
                    ineligible
                        .matches("query_raw (sql . as_str () , values)")
                        .count(),
                    2
                );
            }
            DbCrate::Postgres(_) => {
                ...
                assert!(eligible.contains("(sql . as_str () , & values)"));
            }
        }
...
fn deadpool_caches_every_supported_annotation_and_flag_only_queries() {
    ...
    for (start, end, functions) in [
        ...
        assert!(
            !region.contains("(sql . as_str () , & values)"),
            "{start}: {region}"
        );
        assert!(
            !region.contains("query_raw (sql . as_str ()"),
            "{start}: {region}"
        );
