...
fn build(sql: &str, placeholders: Placeholders, args: &[Arg]) -> (String, Vec<Bind>) {
    compile_with_arg_order(sql, placeholders, &order(args), &[]).build(args)
}
...
fn build_plan(sql: &str, plan: &[Clause<'_>], args: &[Arg]) -> String {
    compile_with_arg_order(sql, Placeholders::Numbered, &order(args), plan)
        .build(args)
        .0
}
...
fn keeps_postgres_question_operators_as_text_with_an_argument_order() {
    ...
    let compiled = compile_with_arg_order(sql, Placeholders::Numbered, &[1, 2], &[]);
    ...
    let (actual, binds) =
        compile_with_arg_order("SELECT a = ? AND b = ?", Placeholders::Question, &[2, 1], &[])
...
fn hash_comments_inside_a_gated_block_stay_inert() {
    ...
    for (args, expected, binds) in states {
        let (actual, actual_binds) =
            compile_with_arg_order(sql, Placeholders::Question, &[1, 2], &plan).build(&args);
