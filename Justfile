...
check-local:
    ...
    just generate
    git diff --exit-code
    cargo test -p sqlc-gen-rust -p authors-sqlx-sqlite -p authors-sqlx-sqlite-params -p sqlc-slice-sqlx-sqlite -p type-mapping-sqlx-sqlite -p embed-sqlx-sqlite -p embed-sqlx-sqlite-params -p embed-rusqlite
