...
static SEARCH_USERS_DYN: std::sync::LazyLock<dynfilter::Compiled> =
    std::sync::LazyLock::new(|| {
        dynfilter::compile(SEARCH_USERS, dynfilter::Placeholders::Question)
    });
...
pub async fn search_users<'e>(
    ...
    for bind in binds {
        q = match bind {
            ...
            dynfilter::Bind::Arg(2usize) => q.bind(params.created_at.as_ref().unwrap()),
            dynfilter::Bind::Elem(4usize, element) => q.bind(&params.ids.unwrap()[element]),
...
WHERE TRUE
  ...
  AND TRUE";
static COUNT_USERS_DYN: std::sync::LazyLock<dynfilter::Compiled> =
    std::sync::LazyLock::new(|| dynfilter::compile(COUNT_USERS, dynfilter::Placeholders::Question));
#[derive(Debug, Clone, Default)]
...
WHERE TRUE
  ...
  AND users.id IN (/*SLICE:ids*/?2) -- :if $2
  AND TRUE";
static TOUCH_USERS_DYN: std::sync::LazyLock<dynfilter::Compiled> =
    std::sync::LazyLock::new(|| dynfilter::compile(TOUCH_USERS, dynfilter::Placeholders::Question));
#[derive(Debug, Clone, Default)]
