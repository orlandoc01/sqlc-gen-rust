...
static SEARCH_USERS_DYN: std::sync::LazyLock<dynfilter::Compiled> =
    std::sync::LazyLock::new(|| {
        dynfilter::compile(SEARCH_USERS, dynfilter::Placeholders::Numbered)
    });
...
pub async fn search_users<'e>(
    ...
    for bind in binds {
        q = match bind {
            ...
            dynfilter::Bind::Arg(3usize) => q.bind(params.ids.as_ref().unwrap()),
...
static COUNT_USERS_DYN: std::sync::LazyLock<dynfilter::Compiled> =
    std::sync::LazyLock::new(|| dynfilter::compile(COUNT_USERS, dynfilter::Placeholders::Numbered));
#[derive(Debug, Clone, Default)]
...
pub async fn count_users<'e>(
    ...
    for bind in binds {
        q = match bind {
            dynfilter::Bind::Arg(0usize) => q.bind(params.email.as_ref().unwrap()),
            dynfilter::Bind::Arg(1usize) => q.bind(params.ids.as_ref().unwrap()),
            _ => unreachable!("dynfilter bind plan referenced an unknown argument"),
        };
    }
    let q = q.persistent(false);
...
static TOUCH_USERS_DYN: std::sync::LazyLock<dynfilter::Compiled> =
    std::sync::LazyLock::new(|| dynfilter::compile(TOUCH_USERS, dynfilter::Placeholders::Numbered));
#[derive(Debug, Clone, Default)]
