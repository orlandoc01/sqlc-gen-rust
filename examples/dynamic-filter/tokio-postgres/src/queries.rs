...
fn search_users_query<'p>(
    ...
    String,
    Vec<(
        &'p (dyn tokio_postgres::types::ToSql + ::std::marker::Sync),
    ...
    let values = binds
        .iter()
        .map(|bind| match bind {
            dynfilter::Bind::Arg(0usize) => (&params.email as _, tokio_postgres::types::Type::TEXT),
            dynfilter::Bind::Arg(1usize) => (&params.phone as _, tokio_postgres::types::Type::TEXT),
            dynfilter::Bind::Arg(2usize) => {
                (&params.orders_since as _, tokio_postgres::types::Type::TEXT)
            }
            dynfilter::Bind::Arg(3usize) => {
                (&params.ids as _, tokio_postgres::types::Type::INT8_ARRAY)
            }
            dynfilter::Bind::Arg(4usize) => {
                (&params.row_limit as _, tokio_postgres::types::Type::INT4)
            }
...
fn search_users_by_profile_query<'p>(
    ...
    let values = binds
        ...
        .map(|bind| match bind {
            dynfilter::Bind::Arg(0usize) => (&params.email as _, tokio_postgres::types::Type::TEXT),
            dynfilter::Bind::Arg(1usize) => {
                (&params.profile as _, tokio_postgres::types::Type::JSONB)
            }
            _ => unreachable!("dynfilter bind plan referenced an unknown argument"),
        })
        .collect();
    ...
    let values = binds
        ...
        .map(|bind| match bind {
            dynfilter::Bind::Arg(0usize) => {
                (&params.new_phone as _, tokio_postgres::types::Type::TEXT)
            }
            dynfilter::Bind::Arg(1usize) => {
                (&params.user_id as _, tokio_postgres::types::Type::INT8)
            }
            _ => unreachable!("dynfilter bind plan referenced an unknown argument"),
        })
        .collect();
...
fn update_user_email_query<'p>(
    ...
    let values = binds
        ...
        .map(|bind| match bind {
            dynfilter::Bind::Arg(0usize) => {
                (&params.new_email as _, tokio_postgres::types::Type::TEXT)
            }
            dynfilter::Bind::Arg(1usize) => (&params.id as _, tokio_postgres::types::Type::INT8),
            dynfilter::Bind::Arg(2usize) => (&params.email as _, tokio_postgres::types::Type::TEXT),
            _ => unreachable!("dynfilter bind plan referenced an unknown argument"),
        })
        .collect();
    (sql, values)
