...
pub async fn list_authors_by_ids<'e>(
    ...
    let mut q = sqlx::query_as::<_, ListAuthorsByIDsRow>(&sql);
...
pub async fn list_authors_by_two_id_lists<'e>(
    ...
    let mut q = sqlx::query_as::<_, ListAuthorsByTwoIdListsRow>(&sql);
    for bind in binds.iter().copied() {
        q = match bind {
            dynfilter::Bind::Elem(0usize, element) => {
                let elem = &params.ids[element];
            ...
            dynfilter::Bind::Elem(1usize, element) => {
                let elem = &params.backup_ids[element];
                q.bind(elem)
            }
            _ => unreachable!("dynfilter bind plan referenced an unknown argument"),
        };
...
pub async fn list_authors_by_ids_mixed<'e>(
    ...
    let mut q = sqlx::query_as::<_, ListAuthorsByIDsMixedRow>(&sql);
    for bind in binds.iter().copied() {
        q = match bind {
            ...
            dynfilter::Bind::Elem(3usize, element) => {
                let elem = &params.skip_ids[element];
                q.bind(elem)
            }
            dynfilter::Bind::Arg(1usize) => q.bind(params.name),
            _ => unreachable!("dynfilter bind plan referenced an unknown argument"),
        };
    }
    let q = q.persistent(false);
...
pub async fn list_authors_by_named_ids<'e>(
...
) -> Result<Vec<ListAuthorsByNamedIDsRow>, sqlx::Error> {
    let args = [
        dynfilter::Arg::Active,
        dynfilter::Arg::Slice(Some(params.ids.len())),
        dynfilter::Arg::Active,
    ...
    let mut q = sqlx::query_as::<_, ListAuthorsByNamedIDsRow>(&sql);
    for bind in binds.iter().copied() {
        q = match bind {
            dynfilter::Bind::Arg(0usize) => q.bind(params.min_id),
            dynfilter::Bind::Elem(1usize, element) => {
                let elem = &params.ids[element];
                q.bind(elem)
            }
            dynfilter::Bind::Arg(2usize) => q.bind(params.max_id),
            _ => unreachable!("dynfilter bind plan referenced an unknown argument"),
        };
    }
    let q = q.persistent(false);
...
impl<'r> sqlx::FromRow<'r, sqlx::sqlite::SqliteRow> for ListAuthorLabelsByIDsRow {
    fn from_row(row: &'r sqlx::sqlite::SqliteRow) -> Result<Self, sqlx::Error> {
        Ok(Self {
            id: sqlx::Row::try_get(row, 0)?,
...
pub async fn list_author_labels_by_ids<'e>(
    executor: impl sqlx::Executor<'e, Database = sqlx::Sqlite>,
    params: ListAuthorLabelsByIDsParams<'_>,
) -> Result<Vec<ListAuthorLabelsByIDsRow>, sqlx::Error> {
    let args = [dynfilter::Arg::Slice(Some(params.ids.len()))];
    let (sql, binds) = LIST_AUTHOR_LABELS_BY_IDS_DYN.build(&args);
    let mut q = sqlx::query_as::<_, ListAuthorLabelsByIDsRow>(&sql);
    for bind in binds.iter().copied() {
        q = match bind {
            dynfilter::Bind::Elem(0usize, element) => {
                let elem = &params.ids[element];
